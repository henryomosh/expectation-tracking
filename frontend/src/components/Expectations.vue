<template>
  <div
    ...
    class="float-right bg-green-200 rounded-lg py-5 px-6 mb-3 text-base text-green-900 inline-flex fixed right-2 top-20 z-10"
    role="alert"
  >
    {{ statusMessage }}
  </div>
  <div class="px-10 py-10 my-2 lg:ml-5">
    ...
    <div v-show="loading" class="flex items-center justify-center mt-5">
      <span
        class="
          spinner-border
          animate-spin
...
export default {
  ...
  methods: {
    ...
    deleteCurrentExpectation(expectation) {
      deleteExpectation(expectation.id).then(() => {
        this.expectations = this.expectations.filter((element) => {
          this.$store.commit("status/statusMessage", "Expectation deleted!");
    ...
    resetStatus() {
      setTimeout(() => {
        this.$store.commit("status/statusMessage", "");
      }, 5000);
    }
  }
};
